...
def test_extract_title_html_entities_unescaped():
    ...
    assert (
        module._extract_title(content, "stem_name") == "Title & with <tags> & 'quotes'"
    )
...
def test_extract_description_meta_tag():
    ...
    assert (
        module._extract_description(content2, "test.html", {}) == "Another description"
    )
...
def test_extract_description_subtitle():
    ...
    assert (
        module._extract_description(content2, "test.html", {})
    ...
    assert (
        module._extract_description(content3, "test.html", {}) == "Spaced out & entity"
    )
...
def test_extract_description_fallback_dict():
    ...
    assert (
        module._extract_description(content, "test.html", descriptions)
...
def test_atom_entry_escapes_url_and_id():
    ...
    assert (
        '<link href="https://example.com/page?param=1&amp;other=&lt;script&gt;" />'
        in entry_xml
    )
    assert (
        "<id>https://example.com/page?param=1&amp;other=&lt;script&gt;</id>"
        in entry_xml
    )
    assert "https://example.com/page?param=1&other=<script>" not in entry_xml


def test_batch_git_iso_success():
    module = load_generate_feed_module()

    mock_files = [Path("/app/file1.html"), Path("/app/file2.html")]

    with patch("subprocess.run") as mock_run:
        mock_run.return_value.stdout = (
            "TS:2025-01-01T12:00:00Z\nfile1.html\n\nTS:2024-12-01T12:00:00Z\nfile2.html"
        )
        timestamps = module._batch_git_iso(mock_files)

        assert len(timestamps) == 2
        assert timestamps[Path("/app/file1.html")] == "2025-01-01T12:00:00Z"
        assert timestamps[Path("/app/file2.html")] == "2024-12-01T12:00:00Z"


def test_batch_git_iso_fallback():
    module = load_generate_feed_module()

    mock_files = [Path("/app/file1.html")]

    with patch("subprocess.run") as mock_run:
        # Simulate exception
        mock_run.side_effect = Exception("git failed")

        with patch("deployment.generate_feed.datetime") as mock_datetime:
            # We must mock the datetime returned value for predictable fallback
            class MockDatetime:
                @classmethod
                def now(cls, tz):
                    class MockNow:
            ...
            mock_datetime.timezone = module.timezone

            timestamps = module._batch_git_iso(mock_files)

            assert len(timestamps) == 1
            # Assuming fallback timestamp structure
            assert timestamps[Path("/app/file1.html")].endswith("Z")
