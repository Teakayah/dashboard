...
def get_all_git_dates(files: list[Path]) -> dict:
    """Return a mapping of filename to 'Mon YYYY' for all given files."""
    git_dates = {}
    if not files:
        return git_dates
    try:
        # Pass all files to git log at once.
        # This will print the timestamp and the filename for all commits affecting these files.
        # To avoid ARG_MAX limits, we chunk the arguments
        chunk_size = 100
        file_args = [str(f) for f in files]

        for i in range(0, len(file_args), chunk_size):
            chunk = file_args[i : i + chunk_size]
            result = subprocess.run(
                ["git", "log", "--format=TS:%ci", "--name-only", "--"] + chunk,
                capture_output=True,
                text=True,
                cwd=str(ROOT),
            )
            ...
            for line in result.stdout.splitlines():
                ...
                if line.startswith("TS:"):
                    stamp = line[3:]
                    current_stamp = datetime.fromisoformat(stamp).strftime("%b %Y")
                elif current_stamp:
                    p = Path(line)
                    if p.name not in git_dates:
                        git_dates[p.name] = current_stamp
    except Exception:
        pass

    # Fill in mtime fallback for files not in git (or if git failed)
    for f in files:
        if f.name not in git_dates:
            git_dates[f.name] = datetime.fromtimestamp(f.stat().st_mtime).strftime(
                "%b %Y"
            )
...
def extract_meta(
    ...
    if git_dates is not None and filepath.name in git_dates:
        date_str = git_dates[filepath.name]
    else:
...
def main(argv: Optional[list[str]] = None):
    ...
    git_dates = get_all_git_dates(html_files)
    ...
    for filepath in html_files:
        ...
        # Extract meta
        meta = extract_meta(
            filepath, content, descriptions=descriptions, git_dates=git_dates
        )
