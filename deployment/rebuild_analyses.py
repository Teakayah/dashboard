...
def _inject_const(html: str, var_name: str, new_value: object) -> tuple[str, bool]:
    ...
    pattern = rf'''
        const\ {re.escape(var_name)}  # Matches 'const ' followed by the exact variable name
...
def _inject_insight(html: str, insight: str) -> tuple[str, bool]:
    ...
    pattern = r'''
        <!--\ insight-inject\ -->   # Matches the opening HTML comment marker
        .*?                         # Non-greedily matches the existing insight content
        <!--\ /insight-inject\ -->  # Matches the closing HTML comment marker
    '''
