...
def _batch_git_iso(filepaths: list[Path]) -> dict[Path, str]:
    """Return a mapping of filepath to ISO 8601 timestamp from git log; fall back to current time."""
    timestamps = {}
    ...
    try:
        args = ["git", "log", "--format=TS:%cI", "--name-only", "--"] + [
        ...
        for line in result.stdout.splitlines():
            ...
            if line.startswith("TS:"):
                ...
                # git output gives paths relative to cwd (which is ROOT)
                p = ROOT / line
                if p not in timestamps:
                    timestamps[p] = current_ts
    except Exception:
        pass
...
def _build_entry(filepath: Path, descriptions: dict, updated: str) -> dict:
...
def _atom_entry(entry: dict) -> str:
    ...
    summary_html = (
        f"&lt;p&gt;{html_lib.escape(entry['summary'], quote=True)}&lt;/p&gt;"
        if entry["summary"]
        else ""
    )
...
def main() -> None:
    ...
    git_timestamps = _batch_git_iso(html_files_list)

    html_files = sorted(
        html_files_list,
        key=lambda p: git_timestamps[p],
        reverse=True,
    )

    entries = [_build_entry(f, descriptions, git_timestamps[f]) for f in html_files]
