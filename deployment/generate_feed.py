...
def _extract_description(content: str, filename: str, descriptions: dict) -> str:
    ...
    m = re.search(
        r'''
        <meta[^>]*                  # Match opening <meta and any preceding attributes
        name=["\']description["\']  # Match the name="description" attribute (single or double quotes)
        [^>]*                       # Match any intermediate attributes
        content=["\'](.*?)["\']     # Group 1: Capture the description content
        ''',
