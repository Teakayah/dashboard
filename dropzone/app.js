...
async function handleFiles(files) {
    ...
    try {
        ...
        // ⚡ Bolt: Process files concurrently to avoid O(N) I/O bottlenecks
        await Promise.all(standaloneFiles.map(file => processFile(file, file.name)));
        ...
        for (const [dirName, dirFiles] of Object.entries(fileGroups)) {
            ...
            // ⚡ Bolt: Concurrent file buffering drastically reduces load times for partitioned datasets
            ...
            if (isDelta) {
                ...
                // ⚡ Bolt: Process fallback individual files concurrently
                await Promise.all(dirFiles.map(file => processFile(file, file.webkitRelativePath || file.name)));
            }
        }
