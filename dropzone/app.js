...
const MANUAL_BUNDLES = {
  mvp: {
    mainModule: new URL("./vendor/duckdb/duckdb-mvp.wasm", import.meta.url)
      .href,
    mainWorker: new URL(
  ...
  eh: {
    ...
    mainWorker: new URL(
      "./vendor/duckdb/duckdb-browser-eh.worker.js",
      import.meta.url,
    ).href,
  },
};
...
const initProgressContainer = document.getElementById(
...
function addToHistory(sql) {
  ...
  if (!trimmed) return;
  queryHistory = [trimmed, ...queryHistory.filter((q) => q !== trimmed)].slice(
    0,
    10,
  );
...
loadRemoteDeltaBtn.addEventListener("click", async () => {
  ...
  if (!window.deltaSupported) {
    showToast(
      "Delta Lake support is not available in this browser environment. Please use CSV, JSON, or Parquet files instead.",
    );
    return;
  }
  ...
  try {
    ...
    setTimeout(() => {
      loadRemoteDeltaBtn.textContent = originalText;
    }, 2000);
  } catch (err) {
    console.error(err);
...
function showInitError(message) {
  ...
  btn.style.cssText =
    "margin-left:8px;padding:2px 8px;cursor:pointer;font-size:inherit";
...
async function init() {
  ...
  const timeoutId = setTimeout(() => {
    if (!db) {
      timedOut = true;
      showInitError(
        "DuckDB initialization timed out. Service worker may be stale.",
      );
    }
  }, INIT_TIMEOUT_MS);
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "Persistent db.open failed, falling back to in-memory:",
        err,
      );
    ...
    try {
      ...
      console.warn(
        "Delta extension not supported in this environment:",
        e.message,
      );
...
async function displayTableSchema(tableName) {
  ...
  const cols = getRows(schemaResult).map((r) => {
    ...
    span.setAttribute(
      "aria-label",
      `Insert column ${r.column_name} into SQL editor`,
    );
    ...
    const triggerAction = async (e) => {
      ...
      try {
        ...
        const profilingResult = await conn.query(
          `SELECT MIN("${escapeId(r.column_name)}") as min_val, MAX("${escapeId(r.column_name)}") as max_val, COUNT("${escapeId(r.column_name)}") as count_val FROM "${escapeId(tableName)}"`,
        );
        const stats = getRows(profilingResult)[0];

        const distResult = await conn.query(
          `SELECT "${escapeId(r.column_name)}" as val, count(*) as cnt FROM "${escapeId(tableName)}" GROUP BY 1 ORDER BY 2 DESC LIMIT 10`,
        );
...
async function updateChartBuilderUI() {
  ...
  try {
    const schemaResult = await conn.query(
      `DESCRIBE "${escapeId(currentTableName)}"`,
    );
    ...
    const numericCols = cols.filter((c) =>
      ["DOUBLE", "FLOAT", "BIGINT", "INTEGER", "DECIMAL", "HUGEINT"].includes(
        c.column_type.split("(")[0].toUpperCase(),
      ),
    );
...
async function handleFiles(files) {
  ...
  try {
    ...
    for (const [dirName, dirFiles] of Object.entries(fileGroups)) {
      const isDelta = dirFiles.some((f) =>
        (f.webkitRelativePath || f.name).includes("_delta_log"),
      );
      ...
      if (isDelta) {
        if (!window.deltaSupported) {
          showToast(
            `Delta Lake table detected in folder "${escapeId(dirName)}", but support is missing in this browser. Skipping.`,
          );
          continue;
        }
        currentTableName = tableName;
...
async function generateInstantCharts(tableName) {
  ...
  const textCols = columns
    .filter((c) =>
      ["VARCHAR", "TEXT", "DATE", "TIMESTAMP"].includes(
    ...
    try {
      ...
      for (let i = 0; i < colsToCheck.length; i++) {
        for (let j = i + 1; j < colsToCheck.length; j++) {
          ...
          corrExprs.push(
            `corr("${escapeId(c1)}", "${escapeId(c2)}") as "c_${i}_${j}"`,
          );
      ...
      createPreviewCard(
        `Correlation: ${bestPair[0]} vs ${bestPair[1]}`,
        async (canvasId) => {
          const result = await conn.query(
            `SELECT "${escapeId(bestPair[0])}" as x, "${escapeId(bestPair[1])}" as y FROM "${escapeId(tableName)}" WHERE x IS NOT NULL AND y IS NOT NULL LIMIT 500`,
          );
          const rows = getRows(result);
          renderChart(
            canvasId,
...
downloadBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    await conn.query(
      `COPY (${userQuery}) TO '${csvPath}' (HEADER, DELIMITER ',')`,
    );
...
loadSamplesBtn.addEventListener("click", async () => {
  ...
  try {
    for (const [name, content] of Object.entries(SAMPLE_DATA)) {
      ...
      await conn.query(
        `CREATE TABLE IF NOT EXISTS "${escapeId(tableName)}" AS SELECT * FROM read_csv_auto('${escapedName}')`,
      );
      loadedTables.add(tableName);
      currentTableName = tableName;
    }
    ...
    setTimeout(() => {
      loadSamplesBtn.textContent = originalText;
    }, 2000);
  } catch (err) {
    console.error(err);
...
clearBtn.addEventListener("click", async () => {
  if (
    !confirm(
      "This will permanently delete all loaded tables from your local storage. Continue?",
    )
  )
    return;
...
function showToast(msg) {
  ...
  panel.style.cssText =
    "position:fixed;bottom:20px;right:20px;background:#ef4444;color:#fff;padding:12px 20px;border-radius:8px;z-index:9999;box-shadow:0 4px 6px rgba(0,0,0,0.1);";
