    ...
    await withLoading('Clear Error', async () => {
        ...
        document.getElementById('results').innerHTML = `
            <div class="empty" style="text-align: center; padding: 40px 20px;">
                ...
                <h3 style="font-size: 1.1rem; font-weight: 600; color: var(--text); margin: 0 0 8px 0;">Workspace Empty</h3>
                <p style="font-size: 0.9rem; margin: 0; color: var(--text-muted);">Drop a dataset above or load samples to start querying.</p>
            </div>
        `;
        sqlInput.value = '';
        sqlInput.dispatchEvent(new Event('input'));
