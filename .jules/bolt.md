...
## 2024-05-24 - Avoid Spread Operator on Large Arrays
**Learning:** Using the spread operator (`...values`) with `Math.max` or `Math.min` on large JavaScript arrays can throw a `RangeError: Maximum call stack size exceeded`. Additionally, chaining `.map()` and `.reduce()` creates intermediate array allocations and causes multiple iterations over the data.
**Action:** Replace `Math.max(...values)` / `Math.min(...values)` and array method chains with a single iterative `for` loop to compute aggregations (sum, min, max) simultaneously. This prevents stack overflows and improves iteration performance from O(3N) to O(N) with O(1) space.
