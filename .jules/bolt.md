...
## 2026-05-14 - Avoiding Call Stack Exceeded with Array Aggregation
**Learning:** When computing aggregations like sum, min, and max on large JavaScript arrays (e.g., > 120,000 items), using the spread operator with `Math.max(...values)` or `Math.min(...values)` throws a `RangeError: Maximum call stack size exceeded`. This happens because the JS engine attempts to push every array element onto the call stack as an argument to the function. Additionally, `Array.map` and `Array.reduce` chain calls can create unnecessary garbage collection overhead.
**Action:** Use a single iterative `for` loop to manually track and update the `sum`, `min`, and `max` values in one pass over the array. This is O(N) memory-safe and significantly faster than spreading.
