...
## 2026-05-20 - Testing dynamic script execution
Coverage Gap: `if __name__ == "__main__":` blocks and standalone scripts are frequently untested or improperly mocked, leading to coverage drops and missing edge case validation for CLI failures.
Learning: Python's `runpy.run_path` evaluates a script in a completely new, isolated namespace. Consequently, applying standard `unittest.mock.patch` decorators targeting imported module functions (like `patch("deployment.generate_index.main")`) prior to calling `runpy` does NOT affect the executed script's behavior because the script recompiles the namespace from scratch.
Assertion: When using `runpy` to gather coverage on a module's main entry point, you must either mock lower level built-ins broadly (e.g. `sys.argv`, `sys.exit`) to prevent test exit, or mock target functions *inside* the execution context. An effective alternative is to import the script using Pytest's standard fixture capabilities or `importlib`, and then test the `main()` function directly as a standard unit test.
