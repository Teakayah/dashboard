...
## 2026-05-19 - Testing runpy execution of __main__ blocks
Coverage Gap: The `__main__` block in `deployment/generate_feed.py` was untested, and patching the module's `main` function failed when using `runpy.run_path`.
Learning: To test standalone Python script entrypoints (`if __name__ == '__main__':`) in pytest using `runpy.run_path`, do not attempt to patch functions on the imported module (like `patch.object(module, 'main')`), as `runpy` executes the script in a separate namespace.
Assertion: Instead, mock `sys.exit` (with `side_effect=SystemExit`) and globally mock the underlying side-effecting functions (e.g., `patch('pathlib.Path.write_text')`) to safely verify that the script execution path reaches the expected IO operations without mutating real files.
