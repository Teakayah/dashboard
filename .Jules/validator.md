...
## 2026-06-08 - Mocking Feature Flags for External Dependencies
Coverage Gap: Missing test coverage for the "Load Remote Table" functionality (Delta Lake support) in the dropzone. E2E tests relying on external live network calls are forbidden.
Learning: Features relying on experimental or external integrations (like `httpfs` for remote Delta Lake loads) can be tricky to test without actual network dependencies.
Assertion: Instead of doing a live call, override feature flags or mock global environment state like `window.deltaSupported = false;` in the Playwright DOM before interacting with the UI. This verifies the application behaves predictably (e.g., handles errors gracefully) when an integration isn't present or fails.
