...
## 2026-07-25 - [Mocking Subprocess to Cover Git Helper Functions]
Coverage Gap: The `deployment/git_utils.py` functions natively interacting with `subprocess.run` to call git lack branch coverage for error boundaries and normal standard output parsing, resulting in 53% line coverage on essential build dependencies.
Learning: Git environment dependencies in helper utilities are often untestable if actual repositories aren't seeded correctly in test environments. Explicitly mocking `subprocess.run` via `patch('subprocess.run', return_value=...)` allows for deterministic branch execution of both successful output parsing and caught exceptions.
Assertion: When testing Git utilities handling multiple file batches, always inject a `MagicMock()` mimicking `result.stdout` to test parsing logic and `side_effect=Exception` to confirm resilient fallbacks.
