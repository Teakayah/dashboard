...
## 2026-09-08 - Intercepting Content-Security-Policy Headers for Playwright Axe Injections
Coverage Gap: `test_page_has_no_critical_or_serious_violations` was skipped because the Playwright page failed to inject Axe due to a strict Content-Security-Policy metadata block inside the tested pages.
Learning: In Playwright tests, if injecting external scripts (like `axe-core` from a CDN via `page.add_script_tag()`) is blocked by a strict Content-Security-Policy (CSP) meta tag, use `page.route()` to intercept the HTML response, strip the CSP tag using `re.sub`, and fulfill the modified response before calling `page.goto()`.
Assertion: We intercepted `.html` route requests with Playwright's `page.route` to dynamically strip CSP `meta` tags out of the response body before testing for accessibility violations.
