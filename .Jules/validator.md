...
## 2026-06-03 - Cover StopIteration in CSV Reader
Coverage Gap: The `except StopIteration` block in `_read_csv_stripped` inside `scripts/benchmark_final_test.py` was not covered by tests, leaving edge cases for empty files unverified.
Learning: To properly test for and trigger a `StopIteration` exception when testing a loop that iterates over lines of a file using `next(csv.reader)`, we must mock the file open using `patch('builtins.open', mock_open(read_data=''))` with an empty string. This ensures the reader fails immediately on the first iteration.
Assertion: Used `mock_open(read_data='')` to simulate an empty CSV file and asserted that `_read_csv_stripped` gracefully catches the `StopIteration` and returns an empty list `[]`.
