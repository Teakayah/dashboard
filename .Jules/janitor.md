...
## 2026-08-20 - Leftover duplicated DOM manipulation removed
**Learning:** Found leftover manual DOM manipulation for `select` element options via `document.createElement` that was skipped in a prior consolidation pass.
**Action:** Replaced the duplicated code with the global `populateSelect` utility function in `dropzone/app.js` to ensure consistency and reduce code bloat.
