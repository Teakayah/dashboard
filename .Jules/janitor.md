...
## 2026-09-10 - Automated duplicate test cleanup
Learning: Duplicate test definitions (e.g. `def test_run_query_keyboard_shortcut`) hide test execution and cause linter failures (`F811`).
Action: Use `ruff check . --fix --unsafe-fixes` to clean up some redefinitions, and manually remove duplicate test blocks if ruff fails to clean them fully.
