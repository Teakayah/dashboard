...
## 2026-06-04 - Fix URL validation and add CSP
Learning: When implementing a Content Security Policy (CSP), if the HTML file utilizes an inline `<script type="importmap">`, the `script-src` directive must explicitly include `'unsafe-inline'`. Modern browsers treat inline import maps as inline scripts and will block them, breaking module loading, if this directive is omitted. Also, when fixing Bandit B310 on hardcoded `https://` URLs, runtime checks are logically dead code but `# nosec B310` is sufficient to appease the linter without adding dead code.
Action: Add `'unsafe-inline'` to `script-src` and `style-src` when adding CSP to files with inline import maps and inline styles to prevent breaking functionality. Avoid adding redundant `url.startswith` runtime checks for static, developer-controlled, hardcoded URLs when fixing B310; just use `# nosec B310`.
