...
## 2026-09-14 - Redefined tests and ruff auto-fix dangers
**Learning:** Found a redefined test function in `tests/test_dropzone.py` which triggers `ruff` F811 and silently disables tests. Discovered that using `ruff check . --fix --unsafe-fixes` to globally resolve unused imports (F401) is dangerous because it often scrambles module-level import formatting, replacing clean blocks (e.g. `from helpers import (A, B)`) with multiple ugly single-line imports. This leads to degraded maintainability and I001 format errors.
**Action:** When cleaning up shadowed tests (F811), delete the redundant block directly using string replacement tools rather than relying on global linter auto-fixes. Avoid using `--fix` broadly for unused imports.
