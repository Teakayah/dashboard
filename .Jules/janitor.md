...
## 2026-08-15 - Janitor Cleanup: Fixed duplicate test definitions
**Learning:** Duplicate test definitions exist that trigger ruff F811 and can hide tests. Cleaned up multiple redundant files and code using `ruff check --fix --unsafe-fixes`.
**Action:** Identified `tests/test_dropzone.py` had a duplicate definition for `test_run_query_keyboard_shortcut`. Also noted a few datetime bugs in `test_update_statcan_data.py`. I can use `ruff check --fix --unsafe-fixes` first to grab standard cleanup tasks, and then manually address any `SIM117` and `DTZ011` findings.
