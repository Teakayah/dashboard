...
## YYYY-MM-DD - Dynamic Element Initialization
**Learning:** Static `DOMContentLoaded` initialization scripts (like for fullscreen wrappers) miss dynamically rendered DOM nodes such as canvas elements generated by Chart.js.
**Action:** Utilize `MutationObserver` on `document.body` to intercept added nodes, automatically wiring up required functionality and eliminating the need for downstream components to call initialization hooks manually.
## 2026-09-15 - Unused Imports Clean Up

Learning: Unused and duplicated imports cause clutter, trigger linter warnings (e.g. `F811` redefinition and `F401` unused), and negatively impact readability. However, you must be careful not to conflate redundant or shadowed tests (e.g. `F811`) with unused application logic or simple unused imports. Deleting a test solely to resolve a linter error reduces test coverage and introduces regression risk, which is against the Janitor directive.
Action: Next time, when resolving linter errors such as `F811` or `F401`, focus only on removing true dead code (like unused imports or uncalled functions) using targeted `replace_with_git_merge_diff`. Avoid using linters or auto-fixers like `ruff --fix` for test files if it deletes entire test blocks, as this alters validation behavior and violates the rule to ensure behavior remains identical.
