...
## 2026-07-15 - Empty States for Data Grids
**Learning:** Plain text empty states (like "No results") lack visual hierarchy and can be confusing. Adding structured HTML with hidden decorative icons (`aria-hidden="true"`), bold headings, and helpful next steps improves the user experience and provides better context for screen readers.
**Action:** When designing empty states, always replace plain text with visually structured HTML using existing semantic classes (e.g., `.empty`), including hidden decorative icons and actionable helper text.
