...
## 2024-05-20 - Designing Accessible Empty States for Data Grids
**Learning:** Plain text empty states (like 'No results') often lack visual hierarchy, making them easily overlooked and confusing for screen readers due to a lack of semantic structure.
**Action:** When designing empty states for data grids, always replace plain text with visually structured HTML that includes an explicitly hidden decorative icon (`aria-hidden="true"`), a bold heading, and a helpful description of next steps. Apply existing semantic classes like `.empty`.
