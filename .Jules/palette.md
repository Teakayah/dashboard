...
## 2026-07-07 - Structuring Empty States for Data Grids
**Learning:** Plain text empty states (like "No results") lack visual hierarchy and can confuse users when data grids fail to populate.
**Action:** Always replace plain text empty states with visually structured HTML that includes an explicitly hidden decorative icon (`aria-hidden="true"`), a bold heading, and a helpful description of next steps to provide clear feedback and guidance.
