...
## 2026-06-31 - Empty State Design for Data Grids
**Learning:** Plain text empty states (like "No results") provide poor UX because they lack visual hierarchy and fail to guide the user. Furthermore, decorative elements (like emoji icons) in empty states can confuse screen readers if they are not explicitly hidden.
**Action:** Always replace plain text empty states with visually structured HTML that includes an explicitly hidden decorative icon (`aria-hidden="true"`), a bold heading, and a helpful description of the state and next steps.
