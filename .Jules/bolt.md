...
## 2026-07-12 - [Optimize Grid.js re-rendering to fix memory leak]
**Learning:** Dynamically updating a `gridjs.Grid` component by wiping the DOM container (`.textContent = ''`) and creating a new instance on every data update leaves uncleaned event listeners, causing a severe memory leak and browser slowdown over long sessions.
**Action:** Maintain a reference to the initialized grid and use `gridInstance.updateConfig({...}).forceRender()` to leverage its internal Virtual DOM. Call `gridInstance.destroy()` when completely clearing the data.
