...
## 2026-08-26 - Reduce DuckDB-Wasm IPC Overhead
**Learning:** In DuckDB-Wasm environments, sequentially executing `DROP TABLE IF EXISTS` followed by `CREATE TABLE` introduces redundant IPC roundtrips across the WebWorker boundary.
**Action:** Always use DuckDB's native `CREATE OR REPLACE TABLE` syntax to combine dropping and creating into a single query, significantly reducing IPC overhead.
