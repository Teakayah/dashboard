...
## YYYY-MM-DD - [Batching DuckDB operations for sample data loading]
**Learning:** Sequentially registering files and executing `CREATE TABLE` queries within a loop when loading sample data creates unnecessary O(N) WebWorker IPC roundtrips, slowing down the loading process.
**Action:** When loading multiple sample data files into DuckDB, use `Promise.all` to register the files concurrently, and batch the multiple `CREATE TABLE` statements into a single newline-separated query to minimize IPC roundtrip latency.
