...
## YYYY-MM-DD - [Minimize DuckDB-Wasm IPC with Promise.all]
**Learning:** In DuckDB-Wasm, awaiting independent database operations sequentially (like fetching schemas for multiple tables) creates unnecessary WebWorker IPC latency.
**Action:** Always execute independent database operations concurrently using `Promise.all()` to minimize IPC latency across the WebWorker boundary.
