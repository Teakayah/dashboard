...
## 2026-05-28 - [Optimize Grid.js data mapping]
**Learning:** When passing large datasets to Grid.js, mapping an array of plain objects into an array of arrays (e.g. `rows.map(row => columns.map(col => row[col]))`) adds significant CPU and memory overhead. Grid.js natively supports rendering plain objects if the `columns` configuration uses `{ id: c, name: c }`, bypassing the manual mapping phase.
**Action:** Always map the `columns` configuration to specify `id` keys rather than mapping the dataset itself to arrays.
