...
## 2026-06-05 - Mocking StopIteration in CSV Reader
**Learning:** To properly test for and trigger a `StopIteration` exception when testing a loop that iterates over lines of a file (e.g., calling `next(csv.reader)`), mock the file open using `patch('builtins.open', mock_open(read_data=''))` with an empty string to ensure the reader fails immediately on the first iteration.
**Action:** Always use an empty `mock_open` string when asserting how `StopIteration` is caught.
