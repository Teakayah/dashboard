...
## 2026-08-01 - Testing Chart.js Canvas Exports
**Learning:** File downloads triggered by dynamically generated canvas elements using `canvas.toDataURL()` and `a.download` can be captured and verified with Playwright's `expect_download`. This proves the visual chart builder produces tangible output.
**Action:** Always assert the exported canvas artifact has a non-zero size (`os.path.getsize(dl.path()) > 0`) instead of just assuming the click event successfully encoded the image.
