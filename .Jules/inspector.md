...
## 2026-08-03 - Replace implicit timeouts with auto-waiting assertions
**Learning:** Hardcoded `page.wait_for_timeout` (e.g. `TAB_TIMEOUT`, `300ms`) used for UI animations (tab switches) or async updates (slider event dispatch) causes CI flakiness and slows down tests.
**Action:** Replace explicit sleeps with Playwright's auto-waiting assertions like `expect().to_have_class(re.compile(r'\bactive\b'))` for tab switches, `expect().not_to_have_text()` for slider value updates, and `page.wait_for_function()` for layout stability checks.
