...
## 2024-05-24 - Helper script test coverage
**Learning:** Helper scripts that run locally for data updates, such as `deployment/rebuild_analyses.py`, often interact heavily with the filesystem (`pathlib.Path`). Testing these scripts requires patching class methods on `pathlib.Path` globally, rather than patching instances.
**Action:** When adding coverage for file operations in these helper scripts, ensure proper patching of `pathlib.Path.exists`, `pathlib.Path.read_text`, and `pathlib.Path.write_text` to verify read, write, missing files, and no-change scenarios.
